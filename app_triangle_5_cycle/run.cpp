...
public:
    ...
    virtual void finishFinal_udf(unsigned long long & collector)
    {
    	...
    	if(_my_rank == MASTER_RANK) cout<<"5-Cycle_Triangle Count = "<<sum<<endl;
    	collector = sum;
    }
};
...
public:
    ...
    virtual bool compute(SubgraphT & g, ContextT & context, vector<VertexT *> & frontier)
    {
        ...
        else // context == 3, count
        {
            ...
            {
                if (root.value.adj[j].id <= root.id) continue; // make root become the smallest node
                VertexT * u = g.getVertex(root.value.adj[j].id);

                for (int m = j+1; m < root.value.adj.size(); m++)
                {
                    GMatchQ.push_back(root.value.adj[j].id);
                    GMatchQ.push_back(root.value.adj[m].id);
                    VertexT * v = g.getVertex(root.value.adj[m].id);
                    for (int k = 0; k < u->value.adj.size(); k++)
                    {
                        if (find(GMatchQ.begin(), GMatchQ.end(), u->value.adj[k].id) != GMatchQ.end()) continue;
                        if (u->value.adj[k].id > root.id) continue;
                        VertexT * w = g.getVertex(u->value.adj[k].id);
                        GMatchQ.push_back(u->value.adj[k].id);
                        for (int l = 0; l < v->value.adj.size(); l++)
                        {
                            if (find(GMatchQ.begin(), GMatchQ.end(), v->value.adj[l].id) != GMatchQ.end()) continue;
                            if (v->value.adj[l].id > root.id) continue;
                            VertexT * x = g.getVertex(v->value.adj[l].id);
                            GMatchQ.push_back(v->value.adj[l].id);
                            for (int l1 = 0; l1 < w->value.adj.size(); l1++)
                            {
                                if (x->id == w->value.adj[l1].id)
                                {
                                    // count++;
                                    break;
                                }
